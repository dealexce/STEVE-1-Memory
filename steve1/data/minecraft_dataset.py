...
def env_obs_to_agent(frame, goal_embed, t):
    """
...
class MinecraftDataset(Dataset):
    def __init__(self, episode_dirnames, T, min_btwn_goals, max_btwn_goals,
        ...
        self.episode_dir2idx = {episode_dir: idx for idx, episode_dir in enumerate(episode_dirnames)}
        self.embeds_cache = {}
        for episode_dir, idx in self.episode_dir2idx.items():
            episode = EpisodeStorage(episode_dir)
            embeds = episode.load_embeds_attn()
            def create_memory_embeds(embeds):
                # 15 is the number of embeds that are None
                memory_embeds = np.array(embeds[15:]).squeeze(1)
                memory_embeds = np.concatenate((np.zeros((self.len_memory+15, memory_embeds.shape[1])), memory_embeds))
                return memory_embeds
            self.embeds_cache[idx] = create_memory_embeds(embeds)

    def __len__(self):
        return len(self.episode_chunks)

    # TODO: Modify this to return some identifier, not only timestep `t`s, so that the model can use the identifier to
    #  access the context memory from an EXISTING variable, in time of O(1)
    def __getitem__(self, idx):
        obs_np, actions_np, firsts_np = \
            self.get_episode_chunk(self.episode_chunks[idx], 
                              self.min_btwn_goals, 
                              self.max_btwn_goals,
                              self.p_uncond,)
    ...
    def get_memory_embeds(self, id_t, len_memory):
        if len_memory > self.len_memory:
            raise ValueError(f"len_memory={len_memory} is greater than the dataset's len_memory={self.len_memory}")
        episode_idx, t = id_t[0].item(), id_t[1]
        return self.embeds_cache[episode_idx][t - len_memory:t]
        

    def get_episode_chunk(self, episode_chunk, min_btwn_goals, max_btwn_goals, p_uncond):
        ...
        episode_idx = self.episode_dir2idx[episode_dirpath]
        embeds = episode.load_embeds_attn()
        ...
        # NOTE: embeds_per_timestep stores the embed of the goal at each timestep of the entire episode
        embeds_per_timestep = []
        ...
        # NOTE: Seemingly the goal embeds are set to zero with probability p_uncond, dk why but there's likely a part in the paper talked about this
        ...
        # NOTE: Though the goal embeds for the entire episode is created, only the [start_timestep:end_timestep] portion is used
        for t in range(start_timestep, end_timestep):
            ...
            obs = env_obs_to_agent(frame, embeds_per_timestep[t].reshape(1, -1), th.tensor((episode_idx, t)).reshape(1, -1))
            obs_list.append(obs)
